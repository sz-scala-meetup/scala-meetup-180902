...
object mp5 extends App {
  ...
  class Cooker(store: Refridge) {
    def cookSauce(qty: Quantity): ReaderT[Task,FoodStore,Quantity] = ReaderT{ foodStore =>
      for {
    ...
    def cookPasta(qty: Quantity): ReaderT[Task,FoodStore,Quantity] = ReaderT{ foodStore =>
      for {
  ...
  val shopping: ReaderT[Task,FoodStore,Unit]  =  for {
